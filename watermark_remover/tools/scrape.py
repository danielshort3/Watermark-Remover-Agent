"""Scraping tool and shared utilities for the Watermark Remover project.

This module exposes the :func:`scrape_music` LangChain tool which locates
sheet‑music images on disk or, when necessary, downloads them from the
internet.  It also houses helper functions and module‑level state used by
the other tools, such as :func:`sanitize_title`, ``SCRAPE_METADATA`` and the
root ``logger``.  Subsequent modules import these objects to ensure
consistent logging and directory naming across the pipeline.
"""
